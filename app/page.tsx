...
export default function Home() {
  ...
  const handleConfirm = async (onClose: () => void) => {
    if (termsAccepted && selectedProduct && selectedClient) {
      try {
        ...
        if (selectedProduct.idcausal_subcategoria === 197) {
          departamento = 13;
        }
        ...
        const response = await fetch(
          "http://127.0.0.1:8000/api/nuevo-ticket-cf",
          {
            method: "POST",
            headers: {
  ...
  const onSubmitotp = async (data: { otp: string }) => {
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/verificar-codigo-fibrapoints",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/obtener-codigo-fibrapoints",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const getProducts = async () => {
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/get-activeproducts/27",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
              ...
              <Card>
                ...
                <CardFooter>
                  <Link
                    isExternal
                    showAnchorIcon
                    href={`/cliente/${selectedClient.contrato}`}
                  >
                ...
                <ModalBody>
                  ...
                  <div className="mt-4 flex items-center">
                    ...
                    <Link
                      isExternal
                      showAnchorIcon
                      href="https://fibramax.ec/wp-content/uploads/2024/11/TERMINOS-Y-CONDICIONES-FIBRAPOINTS.pdfi"
                      className="ml-2"
                    >
                      Términos y condiciones
                    </Link>
